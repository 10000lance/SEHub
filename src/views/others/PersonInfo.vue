		...
		<div v-for="(value, key) in personInfo">
			...
			<el-form-item v-if="keyToName(key)" :label="keyToName(key) + ':'" class="apy-item">
				...
				<el-select 
					...
					:disabled="notChange.indexOf(key) !== -1"
...
export default {
	...
	methods: {
		...
		postChange (){
			...
			let newInfo = this.deepCopy(this.personInfo);
			newInfo.name = encodeURI(newInfo.name);
			api(newInfo, res => {
				if (res.status === 200){
				//提交成功更新原视数据
